...
Useful after touching the model or the LR schedule: two steps prove the forward,
backward and optimizer step still agree about shapes without waiting for MNIST to
converge.

```bash
# directly
deepspeed --num_gpus=1 train_ds.py --max-steps 5
...
uv run runpod/runpod_ctl.py recommend 01_basics/02_convnet
uv run runpod/runpod_ctl.py run 01_basics/02_convnet \
