...
uv run deepspeed --num_gpus=1 train_rnn_deepspeed.py
```
...
Sequence models fail in ways that show up on the very first batch — a hidden-state
shape that only breaks when the sequence is not the length you assumed. Two steps
surface that immediately.

```bash
# directly
deepspeed --num_gpus=2 train_rnn_deepspeed.py --max-steps 5
...
uv run runpod/runpod_ctl.py recommend 01_basics/04_rnn
uv run runpod/runpod_ctl.py run 01_basics/04_rnn \
