#!/bin/bash
# SLURM batch script — Groupwise Ranking
#
...
#SBATCH --gres=gpu:2
# TWO GPUs, and not because the model needs them -- the largest scorer here is
# 68k parameters. Two ranks is the smallest number that exercises the point:
# groupwise scoring is sharded by QUERY, never by document, because a group
# split across devices scores each document against a SUBSET of its real
# candidates -- silently, with no error. One rank would never test that.
...
#SBATCH --job-name=groupwise_ranking
...
#SBATCH --output=logs/groupwise_rankin_%j.out
#SBATCH --error=logs/groupwise_rankin_%j.err
...
# No downloads: this example generates its data with numpy. HF_HOME is set
# anyway so the folder behaves like its siblings if you add a real corpus.
...
deepspeed --num_gpus="${NUM_GPUS}" train_groupwise_ranking.py \
