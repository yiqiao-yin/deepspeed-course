...
**Model:** `openai/gpt-oss-20b` · **Dataset:** `HuggingFaceH4/Multilingual-Thinking` · **Example:** `03_llms/08_gpt_oss_lora/lora`
...
```bash
cd 03_llms/08_gpt_oss_lora/lora
...
**`torch_dtype=torch.bfloat16`.** Weights load directly in BF16 — 40 GB rather than 80 GB in FP32, and no loss scaling needed. See [BF16 over FP16](/docs/tutorials/llms/overview#bf16-over-fp16-for-llms).
...
| Setting | Why |
|---|---|
| **Stage 2, not 3** | LoRA. Stage 3 would gather 40 GB of frozen expert weights every forward and backward to partition a few hundred MB of trainable state — see [the LoRA note](/docs/tutorials/llms/overview#3-choosing-a-strategy-from-parameter-count) |
| `offload_optimizer: none` | The optimizer state is LoRA-sized. Offloading it would add PCIe traffic to save nothing |
| All keys `"auto"` | Resolved from `TrainingArguments` by `Trainer` — [see the mechanism](/docs/tutorials/llms/overview#2-the-auto-mechanism) |
