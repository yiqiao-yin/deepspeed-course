...
**Model:** `unsloth/Llama-3.2-3B-Instruct` · **Example:** `03_llms/01_llm_finetuning`
...
[TRL Function Calling](/docs/tutorials/llms/trl-function-calling) for
loss masking and [GRPO](/docs/tutorials/llms/grpo-training) for RL.
:::
...
machinery — see [why BF16](/docs/tutorials/llms/overview#bf16-over-fp16-for-llms).
...
Unusually for this repository, `03_llms/01_llm_finetuning` ships a `requirements.txt` with
exact pins:
...
| Completion-only loss masking | The dataset is Q&A; training on the question wastes signal. See [loss masking](/docs/tutorials/llms/trl-function-calling#4-completion-only-loss-masking) |
...
- [TRL Function Calling](/docs/tutorials/llms/trl-function-calling) — chat templates and completion-only loss masking
