...
**Model:** Qwen-1.5B · **Example:** `03_llms/09_multi_agency`

:::warning Read this as a research sketch, not a recipe
This is an exploratory example rather than a validated pipeline. The value is in the design questions it raises. If you want a production GRPO setup, use [GRPO Training](/docs/tutorials/llms/grpo-training).
...
The underlying idea is reasonable and connects directly to the [degenerate-group problem](/docs/tutorials/llms/grpo-worked-example#3-the-degenerate-group-problem). GRPO learns only when a group's rewards **differ**; a group where all $G$ rollouts agree contributes exactly zero gradient, and
...
- [GRPO Training](/docs/tutorials/llms/grpo-training) — the correct objective, verifiable rewards, and a working configuration
- [GRPO: Worked Numerical Example](/docs/tutorials/llms/grpo-worked-example) — the degenerate-group analysis this example is implicitly trying to address
