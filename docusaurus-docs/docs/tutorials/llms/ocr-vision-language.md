...
**Model:** `Qwen/Qwen2-VL-2B-Instruct` · **Example:** `03_llms/03_ocr`
...
Loss masking applies here exactly as in [the SFT page](/docs/tutorials/llms/trl-function-calling#4-completion-only-loss-masking), and matters more: you never want gradient on the image tokens or the instruction, only on the assistant's answer.
...
`03_llms/03_ocr/run_modern_ocr.py` measures five on the same pages.
...
- [TRL Function Calling](/docs/tutorials/llms/trl-function-calling) — chat templates and loss masking in the text-only case
