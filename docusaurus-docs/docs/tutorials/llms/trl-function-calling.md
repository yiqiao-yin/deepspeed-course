...
**Model:** `Qwen/Qwen3-0.6B` · **Task:** emit well-formed tool calls · **Example:** `03_llms/02_trl_sft`
...
Since `weight_decay` is `"auto"`, this config expects HF `Trainer` to resolve it — which `SFTTrainer` provides, as it subclasses `Trainer`. See [the `"auto"` mechanism](/docs/tutorials/llms/overview#2-the-auto-mechanism).
:::
...
| [DPO](/docs/tutorials/llms/grpo-training#3-direct-preference-optimization--the-other-escape-route) | Preference pairs | Overkill — there is no meaningful "preferred" among valid calls |
| [GRPO](/docs/tutorials/llms/grpo-training) | A reward function | Possible — schema validity is verifiable — but SFT is far cheaper |
