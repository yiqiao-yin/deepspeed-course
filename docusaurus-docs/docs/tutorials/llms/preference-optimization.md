...
**Example folder:** [`03_llms/05_dpo/`](https://github.com/yiqiao-yin/deepspeed-course/tree/main/03_llms/05_dpo) — one `train_dpo.py` with `--method dpo|ipo|cpo|kto|orpo|simpo`, plus `preference_losses.py` (all six losses on plain tensors, no GPU, no download).
...
```bash
uv run 03_llms/05_dpo/preference_losses.py
