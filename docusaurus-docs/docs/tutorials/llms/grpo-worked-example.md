...
A companion to [GRPO Training](/docs/tutorials/llms/grpo-training). Everything here is computed on toy or simulated data so the mechanics are fully inspectable — no model, no GPU, no training run.
...
- [GRPO Training](/docs/tutorials/llms/grpo-training) — the full derivation and the DeepSpeed implementation
