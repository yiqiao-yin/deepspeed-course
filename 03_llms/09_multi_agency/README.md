...
This one drives TRL's `GRPOTrainer` directly rather than the DeepSpeed launcher
(see the note in `CONTRIBUTING.md`), but the flag behaves identically.

```bash
# directly
python train_grpo_math.py --max-steps 5

# through the launcher — it forwards its arguments, so this works on SLURM too
sbatch run_slurm.sh --max-steps 5
```
...
uv run runpod/runpod_ctl.py recommend 03_llms/09_multi_agency
uv run runpod/runpod_ctl.py run 03_llms/09_multi_agency \
