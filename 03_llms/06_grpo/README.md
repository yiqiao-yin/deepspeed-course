...
GRPO generates completions every step, so it is slow per step and expensive to get
wrong. A capped run exercises generation, the reward function and the ZeRO config
together — which is precisely the combination that breaks.

```bash
# directly
deepspeed --num_gpus=2 grpo_gsm8k_train.py --max-steps 5
...
[Preference Optimization](https://yiqiao-yin.github.io/deepspeed-course/docs/tutorials/llms/preference-optimization)
and [Beyond GRPO](https://yiqiao-yin.github.io/deepspeed-course/docs/tutorials/llms/beyond-grpo).
...
**Solution:**
Ensure you're running the script from the `03_llms/06_grpo` folder where `ds_config.json` is located:
```bash
cd 03_llms/06_grpo
uv run deepspeed --num_gpus=2 grpo_gsm8k_train.py
```
