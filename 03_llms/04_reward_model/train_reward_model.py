...
Most alignment work now skips this entirely -- `03_llms/05_dpo` derives an
...
Three reasons it is still the right call:
  ...
  2. **Best-of-n sampling** needs a scorer at inference time. Only this route
     gives you one.
  3. **Online methods need a judge.** `03_llms/07_online_dpo` and GRPO with
...
RunPod (auto-shutdown): uv run runpod/runpod_ctl.py run 03_llms/04_reward_model \
...
def require_gpu() -> None:
    ...
    print("      uv run 03_llms/04_reward_model/reward_modeling.py")
    print("      uv run tests/test_reward_model.py      # no GPU, no download")
    print("\n  Check your setup:")
    print("      nvidia-smi")
    print("      ds_report")
    print("\n  Rent a GPU (needs RUNPOD_API_KEY):")
    print("      uv run runpod/runpod_ctl.py recommend 03_llms/04_reward_model")
    print("      uv run runpod/runpod_ctl.py run 03_llms/04_reward_model \\")
