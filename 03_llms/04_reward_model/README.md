...
**Book page:** [RLHF and Reward Modeling](https://yiqiao-yin.github.io/deepspeed-course/docs/tutorials/llms/rlhf-reward-modeling)

## Why build one, when DPO deletes it?

[`../05_dpo/`](../05_dpo/) derives an implicit reward
...
3. **Online methods need a judge.**
   [`../07_online_dpo/`](../07_online_dpo/) consumes
...
```bash
cd 03_llms/04_reward_model
uv sync                    # creates .venv, installs the LOCKED versions
uv run deepspeed --num_gpus=1 train_reward_model.py
```
...
uv run runpod/runpod_ctl.py run 03_llms/04_reward_model \
...
- [`../05_dpo/`](../05_dpo/) — skip the reward model
  entirely; usually the better trade.
- [`../07_online_dpo/`](../07_online_dpo/) — use this
  model as the judge.
- [`../06_grpo/`](../06_grpo/) — when a ground-truth
