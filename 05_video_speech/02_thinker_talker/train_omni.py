...
----------------
    ...
    Frontier omni (100B+)     multi-node -- see ../01_longcat_omni/
...
RunPod (auto-shutdown): uv run runpod/runpod_ctl.py run 05_video_speech/02_thinker_talker \\
...
def require_gpu() -> None:
    ...
    print("      uv run 05_video_speech/02_thinker_talker/tmrope.py")
    print("      uv run tests/test_tmrope.py         # 58 checks, no GPU")
    print("\n  Also CPU-runnable in this topic:")
    print("      uv run 05_video_speech/03_duplex_streaming/duplex.py")
    print("      uv run 05_video_speech/04_omni_eval/omni_eval.py")
    print("\n  Check your setup:")
    print("      nvidia-smi")
    print("      ds_report")
    print("\n  Rent a GPU (needs RUNPOD_API_KEY):")
    print("      uv run runpod/runpod_ctl.py recommend 05_video_speech/02_thinker_talker")
    print("      uv run runpod/runpod_ctl.py run 05_video_speech/02_thinker_talker \\")
...
def synthetic_sample(index: int, duration: float = 4.0, fps: float = 2.0
    ...
    repository has already shipped once, in `04_video_text/`.
    """
    import numpy as np
...
def main() -> None:
    ...
    parser.add_argument("--data-dir", default=None,
                        help="Corpus of {in.mp4, in.wav, out.wav}. Defaults to "
                             "the shared 05_video_speech/data/ if present, else "
                             "synthetic.")
    ...
    if data_dir:
        ...
        raise NotImplementedError(
            ...
            "build_position_ids. The shared corpus at 05_video_speech/data/ is the one "
    ...
    def collate(batch: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        nothing raises -- the exact failure `04_video_text/` shipped with. Two streams
