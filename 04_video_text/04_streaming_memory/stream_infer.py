...
WHAT MAKES THIS DIFFERENT FROM EVERY OTHER SCRIPT IN 04_video_text
------------------------------------------------------------
...
`../03_token_compression/` optimises that shape — and every one of them
...
CPU, no model, no download:
    uv run 04_video_text/04_streaming_memory/stream_infer.py --frames 5000

CoreWeave / SLURM:      sbatch run_deepspeed.sh
RunPod (auto-shutdown): uv run runpod/runpod_ctl.py run 04_video_text/04_streaming_memory \
